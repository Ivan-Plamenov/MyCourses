function bookShelf(input) {
  ...
  let sortedGenres = Object.entries(books).sort((bookA, bookB) => {
  ...
  for (const [genre, booksCollection] of sortedGenres) {
    let shelfId = Object.entries(shelves).find(([id, g]) => g === genre)[0];
    console.log(`${shelfId} ${genre}: ${booksCollection.length}`);
    let sortedBooks = booksCollection.sort((bookA, bookB) =>
