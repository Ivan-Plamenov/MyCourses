function lockedProfile() {
  ...
  function checkLock(e) {
    ...
    let unlock = e.currentTarget.parentElement.querySelector(
      'input[value="unlock"]'
    );
    let hiddenInputs = Array.from(
      e.currentTarget.parentElement.querySelectorAll(".hiddenInfo > input")
    );
    let hiddenLabels = Array.from(
      e.currentTarget.parentElement.querySelectorAll(".hiddenInfo > label")
    );
