function solve() {
  ...
  function onAdd(event) {
    ...
    const nameProduct = parentDiv.querySelector(
      ".product-details .product-title"
    ).textContent;
    const price = parentDiv.querySelector(".product-line-price").textContent;

    textArea.value += `Added ${nameProduct} for ${Number(price).toFixed(
      2
    )} to the cart.\n`;
  ...
  function onCheckout() {
    textArea.value += `You bought ${products.join(", ")} for ${totalSum.toFixed(
      2
    )}.`;
