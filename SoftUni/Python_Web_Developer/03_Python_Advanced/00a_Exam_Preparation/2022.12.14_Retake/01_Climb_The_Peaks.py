...
while True:
    ...
    result = food + stamina
    next_peak = all_peaks[0]  # check the first peak without removing it
    if result >= next_peak[1]:
        conquered_peaks.append(next_peak[0])
        all_peaks.popleft()  # remove the conquered peak
    # no else block, because if the peak isn't conquered, we'll try again on the next day
    day += 1
