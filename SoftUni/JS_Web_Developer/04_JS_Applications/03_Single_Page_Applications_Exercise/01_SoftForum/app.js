...
window.onload = async (ev) => {
  ...
  document
    ...
    .addEventListener("click", () => formOnPage.reset());
};
...
async function loadComments() {
  ...
  const commentsArr = Object.entries(comments).filter(
    ([, value]) => value.topicId === topicId
  );
  ...
  commentsArr.forEach((comment) => {
    ...
    component.querySelector(".post-details-comment").textContent =
      comment[1].comment;
...
function getForm() {
  ...
  form.addEventListener(
    "submit",
    isPathHomePage ? makePostAsync : postCommentAsync
  );
